        ...
        <tbody>
          <tr>
            {items.map((item, index) => (
              <td className="numerial-data" key={index}>
                {item.datum}
              </td>
            ))}
          </tr>
...
// Charts
const CasesGrowthFactorInAnAreaLineChart = ({ area }) => {
  ...
  return (
    <>
      <div className="chart-title">{data.areaName}新增確診趨勢</div>
      {isLoading ? (
        <LoadingSpinner />
      ) : (
        <>
...
const ConfirmedCasesChinaVsWorldLineChart = () => {
  ...
  useEffect(() => {
    async function getData() {
      try {
        setIsLoading(true);
        const response = await fetch(`https://covid.mathdro.id/api/daily`);
        let data = await response.json();
        // Shortens date strings to show more on the axis
        data.forEach(
          d =>
            (d.reportDateString = d.reportDateString.replace(/^\d{4}\//g, ""))
        );
        setData(data);
        setIsLoading(false);
      } catch (error) {
  ...
  return (
    <>
      <div className="chart-title">確診病例趨勢</div>
      {isLoading ? (
        <LoadingSpinner />
      ) : (
        <>
          <SmallTable
            items={[
              { heading: "中國", datum: data.slice(-1)[0].mainlandChina },
              {
                heading: "其他國家",
...
const ConfirmedCasesInAreasLineChart = () => {
  const [data, setData] = useState([
    {
      province: "",
      country: "",
  ...
  return (
    <>
      <div className="chart-title">重點地區確診數比較</div>
      <div className="line-chart">
        {isLoading ? (
          <LoadingSpinner />
        ) : (
...
const CurrentInfoByAreasTable = () => {
  ...
  return (
    <>
      <div className="chart-title">前十大確診地區</div>
...
const DeathRateByAgeGroupBarChart = () => {
  const pie_data = [
    { x: "0-9", y: 0.000094 },
    { x: "10-19", y: 0.022 },
    { x: "20-29", y: 0.091 },
  ...
  return (
    <>
      <div className="chart-title">湖北省各年齡層病死率</div>
...
const DeathRateInAnAreaLineChart = ({ area }) => {
  const [data, setData] = useState({ areaName: "", statisticsData: [] });
  const [isLoading, setIsLoading] = useState(true);
  useEffect(() => {
    async function getData() {
  ...
  return (
    <>
      <div className="chart-title">{data.areaName}病死率</div>
      {isLoading ? (
        <LoadingSpinner />
      ) : (
        <>
...
const WorldCasesGrowthFactorLineChart = () => {
  const [{ data, isLoading, isError }, setData] = useMathdroDailyApi();
  const axis_style = {
    grid: {
      stroke: "#f3f5f6",
      strokeWidth: 2,
  ...
  useEffect(() => {
    // Removes latest changing data; we want each day's final number
    data.splice(-1, 1);
    setData(data);
  }, [data, setData]);
  return (
    <>
      <div className="chart-title">全球新增確診趨勢</div>
        ...
        <>
          ...
          <div className="line-chart">
            <V.VictoryChart
              containerComponent={
                <V.VictoryVoronoiContainer
                  labels={({ datum }) => datum["deltaConfirmed"]}
                  labelComponent={<V.VictoryTooltip constrainToVisibleArea />}
                />
              }
              padding={{ top: 20, bottom: 40, left: 50, right: 20 }}
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="overview">
          <p>{data.slice(-1)[0].confirmed.total}</p>
          <p>(+{data.slice(-1)[0].deltaConfirmedDetail.total})</p>
          <p>{data.slice(-1)[0].deaths.total}</p>
          <p>
            +({data.slice(-1)[0].deaths.total - data.slice(-2)[0].deaths.total})
          </p>
        </div>
      )}
    </>
...
const WorldRecoveryProgressBarChart = () => {
  const [data, setData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const pieData = [
    ...
    <>
      ...
      {isLoading ? (
        ...
        <>
          <SmallTable
            items={[
              { heading: "全球確診", datum: data.confirmed },
              { heading: "全球病癒", datum: data.recovered },
              { heading: "全球死亡", datum: data.deaths },
              { heading: "病死率", datum: data.deathRate + "%" }
            ]}
          />
...
export {
  CasesGrowthFactorInAnAreaLineChart,
  ConfirmedCasesInAreasLineChart,
  ConfirmedCasesChinaVsWorldLineChart,
  CurrentInfoByAreasTable,
  DeathRateByAgeGroupBarChart,
  DeathRateInAnAreaLineChart,
  WorldCasesGrowthFactorLineChart,
  WorldOverviewTable,
  WorldRecoveryProgressBarChart
};
